...
void DEWSTOPBUTTON::_PrepareImages()
{
    Graphics        *pGr = NULL;
    RectF           rctPos;
...
DEWSTOPBUTTON::DEWSTOPBUTTON(LPDEWUIOBJECT Parent, const DEWTHEME& Theme)
{
    RECT    rctClient;

    _btObjectID = DEWUI_OBJ_STOP_BUTTON;
    _btObjectType = DEWUI_OBJ_TYPE_MMBUTTON;
    _Parent = Parent;
    StringCchPrintf(_wsText, DEWUI_MAX_TITLE, DEWUI_SYMBOL_STOP);
    _iDPI = Theme.DPI;
    _fScale = Theme.Scale;
...
void DEWSTOPBUTTON::_MoveToDefaultLocation()
{
    int     iX, iY;
    ...
    iX = (int)(DEWUI_DIM_PADDING_X * _fScale + fAlbumArt + fXGap + fPNS + fXGap + fPlay + fXGap + fPNS + fXGap);
...
DEWSTOPBUTTON::~DEWSTOPBUTTON() {} // NOP
