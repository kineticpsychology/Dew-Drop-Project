...
void DEWMINIMIZEBUTTON::_PrepareImages()
{
    Graphics        *pGr = NULL;
    RectF           rctPos;
...
DEWMINIMIZEBUTTON::DEWMINIMIZEBUTTON(LPDEWUIOBJECT Parent, const DEWTHEME& Theme)
{
    RECT    rctClient;

    _btObjectID = DEWUI_OBJ_MIN_BUTTON;
    _btObjectType = DEWUI_OBJ_TYPE_UIBUTTON;
    _Parent = Parent;
    StringCchPrintf(_wsText, DEWUI_MAX_TITLE, DEWUI_SYMBOL_MINIMIZE);
    _iDPI = Theme.DPI;
    _fScale = Theme.Scale;

    this->_Scale(Theme.IconScale);
    GetClientRect((_Parent->Handle), &rctClient);
    _ptPos.x = (rctClient.right - rctClient.left) - _iWidth * 2 - _F(1);
    _ptPos.y = _F(1);
...
void DEWMINIMIZEBUTTON::_MoveToDefaultLocation()
{
    _ptPos.x = _F(DEWUI_DIM_WINDOW_X) - (_iWidth * 2) - _F(1);
    _ptPos.y = _F(1);
    this->_PrepareImages();
...
DEWMINIMIZEBUTTON::~DEWMINIMIZEBUTTON() {} // NOP
