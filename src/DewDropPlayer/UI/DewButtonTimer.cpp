...
void DEWTIMERBUTTON::_PrepareImages()
{
    Graphics        *pGr = NULL;
    RectF           rctPos;
...
void DEWTIMERBUTTON::_MoveToDefaultLocation()
{
    float   fSeq = 2.0f;
    float   fGap;
...
DEWTIMERBUTTON::DEWTIMERBUTTON(LPDEWUIOBJECT Parent, const DEWTHEME& Theme)
{
    RECT    rctClient;

    _btObjectID = DEWUI_OBJ_TIMER_BUTTON;
    _btObjectType = DEWUI_OBJ_TYPE_MMBUTTON;
    _Parent = Parent;
    StringCchPrintf(_wsText, DEWUI_MAX_TITLE, DEWUI_SYMBOL_TIMER);
    _iDPI = Theme.DPI;
    _fScale = Theme.Scale;
...
DEWTIMERBUTTON::~DEWTIMERBUTTON() {} // NOP
