...
void DEWPREVIOUSBUTTON::_PrepareImages()
{
    Graphics        *pGr = NULL;
    RectF           rctPos;
...
void DEWPREVIOUSBUTTON::_MoveToDefaultLocation()
{
    int     iX, iY;
    float   fXGap, fAlbumArt, fPNS, fPlay, fSeekBarTop, fAlbumArtBase;
    ...
    iX = (int)(DEWUI_DIM_PADDING_X * _fScale + fAlbumArt + fXGap);
...
DEWPREVIOUSBUTTON::DEWPREVIOUSBUTTON(LPDEWUIOBJECT Parent, const DEWTHEME& Theme)
{
    RECT    rctClient;

    _btObjectID = DEWUI_OBJ_PREV_BUTTON;
    _btObjectType = DEWUI_OBJ_TYPE_MMBUTTON;
    _Parent = Parent;
    StringCchPrintf(_wsText, DEWUI_MAX_TITLE, DEWUI_SYMBOL_PREV);
    _iDPI = Theme.DPI;
    _fScale = Theme.Scale;
...
DEWPREVIOUSBUTTON::~DEWPREVIOUSBUTTON() {} // NOP
